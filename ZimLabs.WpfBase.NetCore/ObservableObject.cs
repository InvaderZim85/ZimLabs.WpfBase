    ...
    public class ObservableObject : INotifyPropertyChanged
    {
        ...
        protected List<string> GetProperties(PropertyAccessType accessType = PropertyAccessType.Read | PropertyAccessType.Write, bool onlyPublic = true)
        {
            ...
            List<PropertyInfo> properties = (int)accessType switch
            {
                ...
                3 => GetType().GetProperties().Where(w => w.CanRead && w.CanWrite).ToList(),
                _ => GetType().GetProperties().ToList()
            };

            return onlyPublic
                ? properties.Where(w => w.GetMethod?.IsPublic ?? false).Select(s => s.Name).ToList()
